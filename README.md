...
Για την εκτέλεση των πειραμάτων, αρχικά προσπάθησα να χρησιμοποιήσω το κεντρικό μου σύστημα, ένα Lenovo Legion με επεξεργαστή Ryzen 7 4800H και 16GB RAM. Ωστόσο, λόγω της εγκατάστασης των Ubuntu σε VM καμία καμία μετρική του perf επέστρεφε αποτελέσματα όπως φαίνεται και παρακάτω.
...
Προσπάθησα να χρησιμοποιήσω το εργαλείο της AMD, uProf, σκεπτόμενος ότι ίσως το perf να μην είναι compatible με τον επεξεργαστή μου, αλλά μετά από κάποιο πειραματισμό δεν κατάφερα να πάρω αποτελέσματα για τις μετρικές που με ενδιέφεραν. Συνεπώς, αποφάσισα να κάνω μια νέα εγκατάσταση των Ubuntu 24.04 LTS σε ένα παλαιότερο μηχάνημα που είχα, για να τρέξω τα πειράματα χωρίς τη χρήση VM. Παρόλο που οι τεχνικές προδιαγραφές του συστήματος είναι περιορισμένες, η perf λειτουργεί σωστά.
...
#### Βήμα 4Α: Δημιουργία προγραμμάτων memory_stress και memory_stress_lim
...
Το πρόγραμμα `memory_stress_lim.c` δέχεται ως είσοδο το μέγεθος της μνήμης και τον αριθμό των επαναλήψεων που θα εκτελέσει ο βρόχος.
...
```
#### Βήμα 4Β: Εκτέλεση memory_stress_lim με Διαφορετικά Μεγέθη Μνήμης και 1000 Επαναλήψεις

`memory_stress_lim_perf.sh`

Κατά τη διάρκεια των πειραμάτων, εκτελέσαμε το πρόγραμμα `memory_stress_lim` με τρία διαφορετικά μεγέθη μνήμης: 100MB, 200MB και 400MB, με 1000 επαναλήψεις.
...
- Η απόδοση του προγράμματος `memory_stress_lim` είναι εντατική στη χρήση της μνήμης, προκαλώντας υψηλά ποσοστά cache misses.
...
#### Βήμα 4Γ: Αποτελέσματα Πειραμάτων XSBench (3 Διαφορετικά Σενάρια)
...
3. **Επιδράσεις στην Μνήμη Cache και TLBs:**
   - Τα ποσοστά cache misses και TLB load misses υποδεικνύουν ότι το XSBench έχει σημαντικές απαιτήσεις μνήμης, οι οποίες δεν μπορούν να καλυφθούν πλήρως από την υπάρχουσα αρχιτεκτονική της μνήμης cache.
...
Αρχικά απενεργοποιήσαμε τον THP με την εντολή `echo never | sudo tee /sys/kernel/mm/transparent_hugepage/enabled`

Έπειτα τρέξαμε τα δύο scripts `memory_xsbench_same_core.sh` και `memory_xsbench_different_cores.sh`.

Ενεργοποίησαμε ύστερα τον THP με την εντολή `echo always | sudo tee /sys/kernel/mm/transparent_hugepage/enabled` και τρέξαμε ξανά τα δύο scripts.
